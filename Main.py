...
Leakage = False
N_rot = 3100 #np.arange(500,6000+0.001,500) # 3100 Compressor speed - RPM
omega = N_rot/60*2*pi

# Thermodynamic properties at inlet of the compressor
T_in = 303 # Inlet temperature - K
Q_in =  0.39 #np.arange(0.4,0.5+0.0001,0.05)#0.4 # Inlet quality 0.39
P_in = 1e5 # Inlet pressure - Pa - needed if the input is not two-phase, useless otherwise
x_l_in = 0 # oil rate 

#rho_in = np.zeros(len(Q_in))

# Pressure ratio between inlet and outlet
ratio = 5 #np.arange(1.7,5.2 + 0.0001, 0.5)
...
(m_dot_final, eta_is_final, eta_v_final, W_dot_final, CV) = solverF.full_resolution(geo, property_matrix, 
    dict_V, dict_dV, dict_area, T_in, rho_in, x_l_in, P_out, tol_glob, tol_pressure, theta_step, omega, tau_loss, Leakage, Heat_transfer)
