...
class controlVolume:
    ...
    # as an isentropic nozzle     
    def mass_flow_ex(self, property_matrix, geo, P_out, theta_step):    
        ...
        if self.name == 'ddd' or self.name == 'dd':     
            # Evaluation of the discharge port area at the current crank angle
    ...
    def leakages(self, property_matrix, geo, exist, CVs1 = None, CVc1 = None, CVd1 = None, CVddd = None):
        ...
        if self.name == 's1' :
                ...
                A_sa = geoF.radial_area(geo, geo.phi_ie , max( (geo.phi_ie - self.theta), phi_ssa))
...
def full_resolution(geo, property_matrix, dict_V, dict_dV, dict_area, T_in, rho_in, x_l_in, P_out, tol_glob, tol_pressure, theta_step_base, omega, tau_loss, leakage, heat_transfer):
    ...
    while stopping_criteria(geo, CV_c1, CV_ddd, tol_glob):
        ...
        while theta < geo.theta_d - theta_step:
            ...
            CV_ddd.mass_flow_ex(property_matrix, geo, P_out, theta_step) 
        ...
        while abs((CV_d1.P - CV_dd.P)/CV_d1.P) > tol_pressure and theta < geo.theta_d + 2*pi/3:
            ...
            # Mass flow rates through 'ddd' chamber and 's1' chambers are evaluated using their respective method.
            CV_s1.mass_flow_su(property_matrix, geo, rho_in, T_in, x_l_in, theta_step)
            CV_dd.mass_flow_ex(property_matrix, geo, P_out, theta_step)
