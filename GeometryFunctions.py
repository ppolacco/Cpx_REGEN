...
class GeoParam:
    ...
    def __init__(self,phi_i0, phi_is, phi_ie_fix, phi_ie_orb, phi_o0, phi_os, phi_oe_fix, phi_oe_orb, r_b, h_s, D_wall, x_dis0, y_dis0, r_dis, delta_r, delta_f):
        ...
        # The discharge port is charcterized by the coordinates of its centre point and the radius of the hole.
        self.x_dis0 = x_dis0
        self.y_dis0 = y_dis0
        self.r_dis = r_dis
    ...
    def important_values(self):
        ...
        self.V_disp = 2*volume_force_c1(self,0)[1]["V"]/1e9
        V_d1_d = volume_force_d1(self, self.theta_d + 1e-5)["V"]/1e9
...
def volume_force_s1(geo,theta):
    ...
    if theta> 0.001:
        (cx_s1, cy_s1) = polygonCentroid(x_polyS1,y_polyS1)
    # The singular case happening at theta < 0.001 is handled.
    else:
         phi_polyS1_outer = np.linspace(geo.phi_ie_fix  - theta, geo.phi_ie_fix , N_div)
         (x_polyS1_outer, y_polyS1_outer) = geo.getCoord(phi_polyS1_outer, 'i')
         (cx_s1, cy_s1) = (x_polyS1_outer[-1], y_polyS1_outer[-1])
    ...
    dict_s1 = {"V" : V_s1, "dVdTheta": dVdTheta_s1, "xy_poly": xy_polyS1, "cx" : cx_s1, "cy" : cy_s1, "MO_p" : MO_p_s1, "f_vect" : f_vect_s1}
...
def volume_force_s2(geo,theta):   
    ...
    # Centroid is computed by means of the 'polygonCentroid' function if 'theta' > 0.001.
    if theta> 0.001:
        (cx_s2, cy_s2) = polygonCentroid(x_polyS2,y_polyS2)
    # The singular case happening at theta < 0.001 is handled.
    else:
         phi_polyS2_outer = np.linspace(geo.phi_ie_orb  - theta, geo.phi_ie_orb , N_div)
         (x_polyS2_outer, y_polyS2_outer) = geo.getCoord( phi_polyS2_outer, 'i')
         (cx_s2, cy_s2) = (x_polyS2_outer[-1], y_polyS2_outer[-1])
    ...
    dict_s2 = {"V" : V_s2, "dVdTheta" : dVdTheta_s2,"xy_poly": xy_polyS2, "cx" : cx_s2, "cy" : cy_s2, "MO_p" : MO_p_s2, "f_vect" : f_vect_s2}
...
def volume_force_sa(geo,theta):    
    ...
    def volume_SA(geo,theta):
        ...
        return V_sa, xy_polySA1, xy_polySA2  
    
    # Volume of the 'sa' region is computed by means of the dedicated function 'volume_SA'
    (V_sa, xy_polySA1, xy_polySA2) = volume_SA(geo,theta)
    
    # Derivative is computed by means of the definition of infinitesimal different quotient.
    # The infinitesimal increment is chosen as 2pi/360
    h = 2*pi/360
    (V_sa_ph, _, _) = volume_SA(geo,theta+h)
    (V_sa_mh, _, _) = volume_SA(geo,theta-h)
    ...
    dict_sa = {"V" : V_sa, "dVdTheta" : dVdTheta_sa,"xy_polySA1": xy_polySA1 ,"xy_polySA2": xy_polySA2, "MO_p" : MO_p_sa, "f_vect" : f_vect_sa}
    return dict_sa
...
def volume_force_c1(geo,theta):
    ...
    while alpha <= geo.N_c_max:
        ...
        if alpha <= N_c:
            ...
            # Centroid is computed by means of the 'polygonCentroid' function.
            (cx_c1, cy_c1) = polygonCentroid(x_polyC1, y_polyC1)
...
def volume_force_c2(geo,theta):
    ...
    while alpha <= geo.N_c_max:
        ...
        if alpha<= N_c:
             ...
             dict_c1 = volume_force_c1(geo,theta)
...
def volume_force_d1(geo,theta):
    ...
    # Centroid is computed by means of the 'polygonCentroid' function.
    (cx_d1, cy_d1) = polygonCentroid(x_polyD1, y_polyD1)
    ...
    dict_d1 = {"V" : V_d1, "dVdTheta": dVdTheta_d1, "xy_poly": xy_polyD1, "cx" : cx_d1, "cy" : cy_d1, "MO_p" : MO_p_d1, "f_vect" : f_vect_d1}
    
    return dict_d1

# The computation done for chamber 'd1' is repeated for chamber 'd2'
def volume_force_d2(geo,theta):
    """
    Parameters
    ----------
    geo : class geo containing the geometric parameters
    ...
    dict_d1 = volume_force_d1(geo,theta)
    ...
    dict_d2 = {"V" : V_d2, "dVdTheta": dVdTheta_d2, "xy_poly": xy_polyD2, "cx" : cx_d2, "cy" : cy_d2, "MO_p" : MO_p_d2, "f_vect" : f_vect_d2}    
    
    return dict_d2 
  

def volume_force_dd(geo,theta):
    """
    Parameters
    ----------
    geo : class geo containing the geometric parameters
    ...
    # Centroid is computed by means of the 'polygonCentroid' function.
    (cx_dd, cy_dd) = polygonCentroid(x_polyDD, y_polyDD)
    ...
    # Dictionary containing al the informations about the chamber 'd1'. 
    dict_dd = {"V" : V_dd, "dVdTheta": dVdTheta_dd, "xy_poly": xy_polyDD, "cx" : cx_dd, "cy" : cy_dd, "MO_p" : MO_p_dd, "f_vect" : f_vect_dd}
...
def polyShadedPortArea(geo, theta):
    ...
    phi_curve_oo = np.linspace( geo.phi_ie_fix - theta - 2*pi*(geo.N_c_max-1),geo.phi_os, N_div)
    (x_curve_oo, y_curve_oo) = geo.getCoordOrbiting( phi_curve_oo, 'o', theta)   
    ...
    index = findIntersection(x_odis, y_odis, xport, yport)
    
    try: 
        if index[-1,1]> index[0,1]:
             start_angle = 2*pi/len(xport)*(index[0,1] + (index[-1,1] - index[0,1])/2)
        
             (xport, yport) = geo.discharge_port(start = start_angle)
             index = findIntersection(x_odis, y_odis, xport, yport)
    except IndexError: 
        pass
    ...
    if len(index) == 0 or len(index) == 1:
        area_shaded = 0 
        area_port = pi*geo.r_dis**2 - area_shaded
       ...
       L_port = len(xport)
       x_polyShadePort = np.concatenate((xport[index[1,1]:0:-1], xport[L_port:index[0,1]:-1]),axis=None)
       y_polyShadePort = np.concatenate((yport[index[1,1]:0:-1], yport[L_port:index[0,1]:-1]),axis=None)
            
       x_polyShadeScroll =  x_odis[index[0,0] : index[1,0]]
       y_polyShadeScroll =  y_odis[index[0,0] : index[1,0]]
            
       x_polyShade = np.concatenate((x_polyShadePort, x_polyShadeScroll, x_polyShadePort[0]), axis=None)
       y_polyShade = np.concatenate((y_polyShadePort, y_polyShadeScroll, y_polyShadePort[0]), axis=None)

       area_shaded = polygonArea(x_polyShade, y_polyShade)
       area_port = pi*geo.r_dis**2 - area_shaded
        ...
        L_port = len(xport)
        x_polyShadeScroll1 =  x_odis[index[0,0] : index[1,0]]
        y_polyShadeScroll1 =  y_odis[index[0,0] : index[1,0]] 
        
        x_polyShadePort1 = np.concatenate((xport[index[1,1]:index[2,1]:-1]), axis=None)
        y_polyShadePort1 = np.concatenate((yport[index[1,1]:index[2,1]:-1]), axis=None)        
        
        x_polyShadeScroll2 =  x_odis[index[2,0] : index[3,0]]
        y_polyShadeScroll2 =  y_odis[index[2,0] : index[3,0]]        
        
        x_polyShadePort2 = np.concatenate((xport[index[3,1]:0:-1], xport[L_port:index[0,1]:-1]), axis=None)
        y_polyShadePort2 = np.concatenate((yport[index[3,1]:0:-1], yport[L_port:index[0,1]:-1]), axis=None)
        
        x_polyShade = np.concatenate(( x_polyShadeScroll1, x_polyShadePort1,  x_polyShadeScroll2, x_polyShadePort2, x_polyShadeScroll1[0]), axis=None)
        y_polyShade = np.concatenate(( y_polyShadeScroll1, y_polyShadePort1,  y_polyShadeScroll2, y_polyShadePort2, y_polyShadeScroll1[0]), axis=None)
        
        area_shaded = polygonArea(x_polyShade, y_polyShade)
        area_port = pi*geo.r_dis**2 - area_shaded 
    else: 
        theta = theta + 0.01
        area_port = polyShadedPortArea(geo, theta)
      
    return x_polyShade, y_polyShade, area_port  



# def volume_force_ddd(geo,theta):
#     """
#     Parameters
#     ----------
#     geo : class geo containing the geometric parameters
...
#          dict_d1 = volume_force_d1(geo,theta)
#          dict_d1 = volume_force_d1(geo,theta)
#     if (geo.theta_d >= geo.theta_u and (theta >= geo.theta_u and theta <= geo.theta_d)) or (geo.theta_u >= geo.theta_d and (theta >= geo.theta_u or theta <= geo.theta_d)): 
#          dict_d1 = volume_force_d1(geo,theta)
#          dict_d1 = volume_force_d1(geo,theta)
#          dict_d1 = volume_force_d1(geo,theta)      