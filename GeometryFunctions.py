...
from math import pi,sin,cos, sqrt, atan2, atan, acos, tan, ceil
import numpy as np
...
def polygonDefinition(geo, phi_start, phi_int1, phi_int2, phi_end, theta, flag):
    ...
    N_div = activation_function(phi_start, phi_int1)
...
def volume_force_s1(geo,theta,force):
    ...
    (x_polyS1_ph, y_polyS1_ph, _) = polygonDefinition(geo, geo.phi_ie_fix  - theta - h, geo.phi_ie_fix , phi_ssa_ph, geo.phi_ie_fix  - theta - h - pi, theta+h,"1")
    (x_polyS1_mh, y_polyS1_mh, _) = polygonDefinition(geo, geo.phi_ie_fix  - theta + h, geo.phi_ie_fix , phi_ssa_mh, geo.phi_ie_fix  - theta + h - pi, theta-h,"1")
...
def volume_force_s2(geo,theta,force):   
    ...
    (x_polyS2_ph, y_polyS2_ph, _) = polygonDefinition(geo, geo.phi_ie_fix  - theta - h - pi, phi_ssa_ph , geo.phi_ie_orb, geo.phi_ie_fix  - theta - h , theta+h,"2")
    (x_polyS2_mh, y_polyS2_mh, _) = polygonDefinition(geo, geo.phi_ie_fix  - theta + h - pi, phi_ssa_mh , geo.phi_ie_orb, geo.phi_ie_fix  - theta + h , theta-h,"2")
...
def volume_force_sa(geo,theta,force):    
    ...
    def volume_SA(geo,theta):
        ...
        poly_sa1 = Polygon(xy_polySA1)
        poly_sa2 = Polygon(xy_polySA2)
        c_sa1 = poly_sa1.centroid
        c_sa2 = poly_sa2.centroid
        cx_sa1 = c_sa1.x
        cy_sa1 = c_sa1.y
        cx_sa2 = c_sa2.x
        cy_sa2 = c_sa2.y
        cx_sa = (cx_sa1*V_sa1 + cx_sa2*V_sa2)/V_sa
        cy_sa = (cy_sa1*V_sa1 + cy_sa2*V_sa2)/V_sa
...
# The activation function is used to determine the number of point for the discretization
# of polygons
