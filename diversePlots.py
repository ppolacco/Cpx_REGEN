...
def plot_area_port(geo):
    ...
    for i in range(len(theta)):
        (_,_,area_port[i]) = polyShadedPortArea(geo, theta[i])
        (_,_,area_suction[i]) = suctionFlowArea(geo, theta[i])
        (_,_,area_discharge[i]) = area_d_dd(geo,theta[i])
...
def plot_V_dV(geo):
     ...
     for i in range(len(theta)): 
         V_s1_vect[i] = volume_force_s1(geo,theta[i])["V"]
         V_s2_vect[i] = volume_force_s2(geo,theta[i])["V"]
         
         for alpha in range(1,geo.N_c_max + 1,1):
             V_c1_vect[i,alpha-1] = volume_force_c1(geo,theta[i])[alpha]["V"]
             V_c2_vect[i,alpha-1] = volume_force_c2(geo,theta[i])[alpha]["V"]
         V_d1_vect[i] = volume_force_d1(geo,theta[i])["V"]
         V_d2_vect[i] = volume_force_d2(geo,theta[i])["V"]
         V_dd_vect[i] = volume_force_dd(geo,theta[i])["V"]
         
         dV_s1_vect[i] = volume_force_s1(geo,theta[i])["dVdTheta"]
         dV_s2_vect[i] = volume_force_s2(geo,theta[i])["dVdTheta"]
         for alpha in range(1,geo.N_c_max + 1,1):        
             dV_c1_vect[i,alpha-1] = volume_force_c1(geo,theta[i])[alpha]["dVdTheta"]
             dV_c2_vect[i,alpha-1] = volume_force_c2(geo,theta[i])[alpha]["dVdTheta"]
         dV_d1_vect[i] = volume_force_d1(geo,theta[i])["dVdTheta"]
         dV_d2_vect[i] = volume_force_d2(geo,theta[i])["dVdTheta"]
         dV_dd_vect[i] = volume_force_dd(geo,theta[i])["dVdTheta"]
...
def plot_evolution(geo, CV, P_out, P_in):
     ...
     (vect_s1, vect_c1, vect_d1, vect_dd, vect_ddd) = convert_recorded_values(geo, CV_s1, CV_c1, CV_d1, CV_dd, CV_ddd)
...
def basic_plot( y, name_x, name_y, x2 = [None], y2 = [None]):    
...
def PV_diagramm(geo, CV, P_out, P_in):  
     ...
     for i in range(len(theta)):
         V_dd[i] = volume_force_dd(geo,theta[i])["V"]/1e9
         V_ddd[i] = volume_force_dd(geo,theta[i])["V"]/1e9
...
def plot_volumes(geo, CV):
   ...
   for i in range(len(theta_vol)): 
       ...
       V_s1_vect[i] = volume_force_s1(geo,theta_vol[i])["V"]/1e3
      
       
       for alpha in range(1,geo.N_c_max + 1,1):
           V_c1_vect[i,alpha-1] = volume_force_c1(geo,theta_vol[i])[alpha]["V"]/1e3
           
       V_d1_vect[i] = volume_force_d1(geo,theta_vol[i])["V"]/1e3
       V_dd_vect[i] = volume_force_dd(geo,theta_vol[i])["V"]/1e3
